...
- (void)launchDashboardWithBaseURL:(NSURL*)URL completion:(void (^)(id))completion
{
  ...
  dashboard.dashboardDelegate = (NSObject<GreeDashboardDelegate>*)self;
  self.currentDashboard = dashboard;
  
  [self.hostViewController spresentViewController:self.currentDashboard animated:YES completion:nil];
...
- (void)dismissDashboardWithResults:(id)results
{
  if (self.currentDashboard) {
    ...
    [self.hostViewController sdismissViewControllerAnimated:YES completion:nil];
