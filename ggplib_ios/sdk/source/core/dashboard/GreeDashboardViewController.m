...
{  
  ...
  NSString* gameDashboardPathAddParametersUrl = nil;
  ...
  if(parameters!=nil)
  {
    ...
    if ([gameDashboardPath length]!=0) {
      ...
      gameDashboardPathAddParametersUrl = [self dashboardPathString:gameDashboardPath appId:gameDashboardAppId userId:gameDashboardUserId];
      ...
      if ([gameDashboardPath isEqualToString:GreeDashboardModeTop] ||
        ...
        if ([strLeaderboarderId length]!=0) {
          if ([gameDashboardPathAddParametersUrl rangeOfString:@"?"].location == NSNotFound) {
            gameDashboardPathAddParametersUrl = [gameDashboardPathAddParametersUrl stringByAppendingFormat:@"?%@=%@", GreeDashboardLeaderboardId, strLeaderboarderId];
          } else {
            gameDashboardPathAddParametersUrl = [gameDashboardPathAddParametersUrl stringByAppendingFormat:@"&%@=%@", GreeDashboardLeaderboardId, strLeaderboarderId];
          }            
        } else {
          GreeLogWarn(@"Cannot launch the selected dashboard without '%@'", GreeDashboardLeaderboardId);
          gameDashboardPathAddParametersUrl = [self dashboardPathString:GreeDashboardModeTop appId:gameDashboardAppId userId:gameDashboardUserId];
        }
      } else if ([gameDashboardPath isEqualToString:GreeDashboardModeAppSetting]) {
        NSString* applicationIdString = [[GreePlatform sharedInstance].settings objectValueForSetting:GreeSettingApplicationId];
        gameDashboardPathAddParametersUrl = [NSString stringWithFormat:@"%@%@", GreeDashboardModeAppSetting, applicationIdString];
        ...
        gameDashboardPathAddParametersUrl = [NSString stringWithFormat:@"%@%@", GreeDashboardModeUsersInvites, applicationIdString];        
      } else if ([gameDashboardPath isEqualToString:GreeDashboardModeCommunity]) {
        NSString* commmunityId = (NSString*)[parameters objectForKey:GreeDashboardCommunityId];
        NSString* threadId = (NSString*)[parameters objectForKey:GreeDashboardThreadId];
        if (0 < commmunityId.length) {
          gameDashboardBaseURLString = [NSString stringWithFormat:@"http://%@", [[GreePlatform sharedInstance].settings stringValueForSetting:GreeSettingServerUrlDomain]];
          NSMutableString* parametersUrlString = [[NSMutableString alloc] initWithFormat:@"/%@/%@", GreeDashboardModeCommunity, commmunityId];
          if (0 < threadId.length) {
            [parametersUrlString appendFormat:@"/%@", threadId];
          }
          gameDashboardPathAddParametersUrl = [NSString stringWithString:parametersUrlString];
        } else {
          gameDashboardBaseURLString = [NSString stringWithString:[[GreePlatform sharedInstance].settings stringValueForSetting:GreeSettingServerUrlSns]];
          gameDashboardPathAddParametersUrl = [NSString stringWithString:@"/#view=community_updated_list"];
        }
      } else {        
    ...
    gameDashboardPathAddParametersUrl = [NSString stringWithFormat:@"%@", GreeDashboardModeTop];
  }
  
  return [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", gameDashboardBaseURLString, gameDashboardPathAddParametersUrl]];
}
