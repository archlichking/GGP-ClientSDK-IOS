...
- (void)adjustIconsLabelFont
{
  ...
  float boundsWidth = ((UILabel*)[_icons objectAtIndex:0]).bounds.size.width;
  ...
  for (GreeJSSubnavigationIconView *icon in _icons) {
...
- (NSString*)maxIconLabelWidthTextIfOverLimitWidth:(float)limitWidth
{
  ...
  for (GreeJSSubnavigationIconView *icon in _icons) {
    ...
    float width = [text sizeWithFont:defaultFont].width;
    if (width > maxWidth) {
      maxWidthText = text;
      maxWidth = width;
    }
...
- (float)fontSizeWithLimitWidth:(float)limitWidth text:(NSString *)text font:(NSString *)font
{
  ...
  for (; fontSize > kSubnavigationIconLabelFontSizeMinimum && width > limitWidth; fontSize -= fontSizeShrinkStep) {
    width = [text sizeWithFont:[UIFont fontWithName:font size:fontSize]].width;
  }
  return fontSize;
}
