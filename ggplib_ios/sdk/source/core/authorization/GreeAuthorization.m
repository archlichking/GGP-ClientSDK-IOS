...
@synthesize SSOClientRequestUrl = _SSOClientRequestUrl;
  ...
  if (![self isSavedAccessToken]) {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
      if (_enableGrade0 && !_reachabilityIsSet) {
        ...
        //This case is that this app is launched by handling OpenURL but this doesn't login.
        //This process is delayed. So for preventing the conflict with handleOpenURL process,
        //do nothing about the following case.
        NSURL *launchOptionUrl = [self.launchOptions objectForKey:UIApplicationLaunchOptionsURLKey];
        if (([[launchOptionUrl host] isEqualToString:@"authorize"] && [[launchOptionUrl path] isEqualToString:@"/request"])
            || ([[launchOptionUrl host] isEqualToString:kCommandGetAcccesstoken])) {
          //do nothing, but reset launchOptions for recalling authorize.
...
{  
  ...
  _popup.didDismissBlock  = ^(id aSender) {
    ...
    if ([handledCommand isEqualToString:kCommandSSORequire]) {
      ...
      [_httpClient getPath:@"/" parameters:httpParams 
        success:^(GreeAFHTTPRequestOperation *operation, id responseObject) {
          ...
          if (![applist count]) {
            [_popup loadErrorPageOnNotWebAccess:nil];
...
- (void)loadAuthorizePage:(NSMutableDictionary*)params
{
  ...
  NSString* urlString;
  if (_authorizationType == AuthorizationTypeSSOServer) {
    urlString = [NSString stringWithFormat:@"%@/oauth/authorize?context=%@%@",
                 _configServerUrlOpen,
                 context,
                 ([params greeBuildQueryString])?[NSString stringWithFormat:@"&%@",[params greeBuildQueryString]]:@""];
  } else {
    urlString = [NSString stringWithFormat:@"%@/oauth/authorize?oauth_token=%@&context=%@%@",
                 _configServerUrlOpen,
                 requestToken,
                 context,
                 ([params greeBuildQueryString])?[NSString stringWithFormat:@"&%@",[params greeBuildQueryString]]:@""];
  ...
  [_httpClient rawRequestWithMethod:@"GET" path:path parameters:nil 
    success:^(GreeAFHTTPRequestOperation *operation, id responseObject) {
      ...
      if(!key) {
        ...
        //save the requestToken & requestSecret like urlquery (key=secret&key=secret&..)
        ...
        if (requestTokenPairs) {
          [requestTokenPairs setObject:self.userOAuthSecret forKey:self.userOAuthKey];
        } else {
          requestTokenPairs = [NSMutableDictionary dictionaryWithObject:self.userOAuthSecret forKey:self.userOAuthKey];
        }
...
- (void)handleOAuthErrorWithResponse:(id)response
{
  ...
  if ([response isKindOfClass:[NSDictionary class]]) {
    ...
    GreeLog(@"error:%@", dict);
    anErrorString = [NSString stringWithFormat:@"%@[%@]",
  ...
  if (anErrorString) {
    ...
    [_popup loadErrorPageOnNotWebAccess:anErrorString];
  }
}
  ...
  if ([handledCommand isEqualToString:@"authorize"] && [[url path] isEqualToString:@"/request"]) {
    if (self.accessToken && self.accessTokenSecret) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self resetAccessToken];
        [self updateAuthorizationStatus:AuthorizationStatusRequestTokenBeforeGot];
        [self authorizeAction:[NSMutableDictionary dictionaryWithObject:kSelfId forKey:kParamAuthorizationTarget]];
      });
    }
    return;
  }
